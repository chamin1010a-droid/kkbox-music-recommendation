"""
KKBOX Music Recommendation Challenge - LightGBM Pipeline v4b
=============================================================
v4 문제 진단:
  - best_iter=3 → TE 피처가 너무 강력해서 모델이 3번만에 수렴
  - 예측 std=0.017 → 예측이 0.50 근처에 몰림 (변별력 낮음)

v4b 수정:
  1. TE smoothing 대폭 강화 (alpha=10 → alpha=100)
  2. TE를 별도 피처가 아닌 기존 count와 결합한 형태로 사용
  3. early_stopping patience 늘리기
  4. learning_rate 올려서 더 많은 트리 활용
"""
...
SEED = 42
N_FOLDS = 5
SMOOTH_ALPHA = 100  # ★ v4: 10 → v4b: 100 (TE를 훨씬 부드럽게)
np.random.seed(SEED)

log("=" * 65)
log("  KKBOX Music Recommendation - LightGBM Pipeline v4b")
log("  (TE Smoothing 강화 + 학습률 조정)")
log("=" * 65)
...
log(f"  시퀀스/위치 피처 완료 ({time.time()-t0:.1f}초)")
...
# ★ v4b: 하이퍼파라미터 — 더 많은 트리를 학습시키는 방향
params = {
    'objective': 'binary',
    'metric': 'auc',
    'boosting_type': 'gbdt',
    'learning_rate': 0.1,           # ★ v4: 0.03 → v4b: 0.1 (표준 학습률)
    'num_leaves': 255,
    'max_depth': -1,
    ...
    'reg_alpha': 1.0,
    'reg_lambda': 5.0,
    'n_estimators': 1500,           # ★ v4: 3000 → 1500
    'random_state': SEED,
    'verbose': -1,
    'n_jobs': -1,
}

# Target Encoding 그룹 (smoothing 강화)
te_groups = {
    'msno_code': 'user_te',
...
cross_te_groups = [
    ...
    (['msno_code', 'first_genre'], 'user_genre_te'),
    (['msno_code', 'language'], 'user_lang_te'),
]
...
submission.to_csv('submission_v4b.csv', index=False)
log(f"\n  submission_v4b.csv 저장 완료 ({len(submission):,}행)")
...
ax.set_title(f'Feature Importance v4b (Top 25) | OOF AUC: {overall_auc:.6f}',
             fontsize=13, fontweight='bold')
ax.set_xlabel('Average Importance')
plt.tight_layout()
plt.savefig('eda_output/10_feature_importance_v4b.png', dpi=150)
plt.close()

log(f"\n{'='*65}")
log(f"  v4b 파이프라인 완료!")
log(f"  OOF AUC: {overall_auc:.6f}")
log(f"  제출 파일: submission_v4b.csv")
log(f"{'='*65}")
