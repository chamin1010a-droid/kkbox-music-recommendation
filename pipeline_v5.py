...
log("=" * 65)
log("  KKBOX Music Recommendation - LightGBM Pipeline v5")
log("  (TE 완전 제거 - 순수 피처만으로 승부)")
log("=" * 65)
...
log(f"\n  submission_v5.csv 저장 완료 ({len(submission):,}행)")
...
ax.set_title(f'Feature Importance v5 (Top 25) — No TE | OOF AUC: {overall_auc:.6f}',
             fontsize=13, fontweight='bold')
ax.set_xlabel('Average Importance')
plt.tight_layout()
plt.savefig('eda_output/11_feature_importance_v5.png', dpi=150)
plt.close()
